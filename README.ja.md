...
PDFがあらかじめZotomaticに設定したストレージ領域に追加されるたびに、自動検知してノートが作成されます。
...
PDF 監視対象ディレクトリや Zotero 連携設定、LLM 設定などを確認します。
...
指定したディレクトリ内の PDF を一度だけスキャンし、ノートを生成します。
...
PDF 保存ディレクトリを監視し、新しい PDF が追加されるたびにノートを生成します。
...
2. Zotero Connector で論文 PDF を保存
3. PDF が検出され、対応する Markdown ノートが自動生成される

Zotero 管理下の PDF であれば、書誌情報を取得したリッチなノートが生成されます。
...
- 生成された Markdown ノートは、そのまま Obsidian で閲覧・編集できます

---

## LLM 連携について

zotomatic は LLM（現在は ChatGPT）と任意で連携し、論文内容の要約やタグ生成を行えます。
